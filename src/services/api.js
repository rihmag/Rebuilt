...
async function handleResponse(response) {
  ...
  if (!contentType || !contentType.includes('application/json')) {
    ...
    throw new Error(`Server error: ${response.status} ${response.statusText}. Please ensure the API server is running and accessible.`);
  }
...
export async function removeFromTrendingStories(blogId) {
  const response = await fetch(`${API_BASE_URL}/api/trending-stories/${blogId}`, {
    method: 'DELETE',
  });
  return handleResponse(response);
}
